...
var VideoTrimmer = class _VideoTrimmer {
  ...
  async trimWithOptimizedFFmpeg(file, startTime, endTime, options, metadata) {
    try {
      ...
      await ffmpeg.writeFile(inputFileName, await (0, import_util.fetchFile)(file));
  ...
  async loadFFmpeg() {
    ...
    try {
      ...
      await this.ffmpeg.load({
        ...
        wasmURL: await (0, import_util.toBlobURL)(`${baseURL}/ffmpeg-core.wasm`, "application/wasm")
      });
...
var import_ffmpeg2 = require("@ffmpeg/ffmpeg");
var import_util2 = require("@ffmpeg/util");
...
var AudioTrimmer = class {
  ...
  async performAudioTrimming(file, options) {
    ...
    const outputFileName = `output.${outputFormat}`;
    await ffmpeg.writeFile(inputFileName, await (0, import_util2.fetchFile)(file));
    this.emitProgress(30);
    const quality = options.quality || "medium";
  ...
  async loadFFmpeg() {
    ...
    try {
      this.ffmpeg = new import_ffmpeg2.FFmpeg();
      this.ffmpeg.on("progress", ({ progress }) => {
      ...
      const baseURL = "https://unpkg.com/@ffmpeg/core@0.12.6/dist/esm";
      await this.ffmpeg.load({
        coreURL: await (0, import_util2.toBlobURL)(`${baseURL}/ffmpeg-core.js`, "text/javascript"),
        wasmURL: await (0, import_util2.toBlobURL)(`${baseURL}/ffmpeg-core.wasm`, "application/wasm")
      });
      this.ffmpegLoaded = true;
      this.resources.setFFmpeg(this.ffmpeg);
      return this.ffmpeg;
  ...
  (0, import_middleware.persist)(
    (0, import_immer.immer)((set, get) => ({
      ...
      createWorker: (uploadId) => {
        ...
        let worker;
        try {
          worker = new Worker(new URL("../dist/worker/upload.worker.mjs", importMetaUrl), { type: "module" });
        } catch (error) {
          console.warn("[Worker] Trying dev path...");
          try {
            worker = new Worker(new URL("../worker/upload.worker.ts", importMetaUrl), { type: "module" });
          } catch (fallbackError) {
...
var UploadManager = class {
  ...
  constructor(config = {}) {
    this.config = {
      workerUrl: config.workerUrl || new URL("./upload.worker.ts", importMetaUrl).href,
