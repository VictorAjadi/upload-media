...
var CreateH3FileServingHandler = class {
  ...
  async serveFile(ref, event) {
    ...
    try {
      const stat = await import_promises.default.stat(
        fullPath
      );
      if (!stat.isFile()) {
        throw createError({
      ...
      const fileBuffer = await import_promises.default.readFile(
        fullPath
      );
      setHeader(
        event,
  ...
  extractFileId(ref) {
    return import_path.default.basename(ref).replace(
      /\.[^/.]+$/,
      ""
    );
  }
