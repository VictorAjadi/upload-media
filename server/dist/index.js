...
__export(index_exports, {
  ...
  CreateH3FileServingHandler: () => CreateH3FileServingHandler,
  CreateNextjsFileServingHandler: () => CreateNextjsFileServingHandler,
  DEFAULT_CACHE_TTL_SECONDS: () => DEFAULT_CACHE_TTL_SECONDS,
  DEFAULT_CHUNK_SIZES: () => DEFAULT_CHUNK_SIZES,
  DEFAULT_CLEANUP_BATCH_SIZE: () => DEFAULT_CLEANUP_BATCH_SIZE,
  DEFAULT_CLEANUP_INTERVAL_MS: () => DEFAULT_CLEANUP_INTERVAL_MS,
  DEFAULT_QUALITY: () => DEFAULT_QUALITY,
  DEFAULT_SIZE_LIMITS: () => DEFAULT_SIZE_LIMITS,
  DEFAULT_STALE_UPLOAD_RETENTION_MS: () => DEFAULT_STALE_UPLOAD_RETENTION_MS,
  ...
  QUALITY_MAPPINGS: () => QUALITY_MAPPINGS,
  S3StorageAdapter: () => S3StorageAdapter,
  SQLRepository: () => SQLRepository,
  SUPPORTED_MIME_TYPES: () => SUPPORTED_MIME_TYPES,
  THUMBNAIL_CHUNK_SIZE: () => THUMBNAIL_CHUNK_SIZE,
  THUMBNAIL_DIMENSIONS: () => THUMBNAIL_DIMENSIONS,
  THUMBNAIL_SIZE_LIMIT: () => THUMBNAIL_SIZE_LIMIT,
  UploadEngine: () => UploadEngine,
  ValidationError: () => ValidationError,
  assertKindAllowed: () => assertKindAllowed,
  assertRequiredFields: () => assertRequiredFields,
  assertWithinLimit: () => assertWithinLimit,
  chainHooks: () => chainHooks,
  createAutoCleanupHooks: () => createAutoCleanupHooks,
  createElysiaAdapter: () => createElysiaAdapter,
  createElysiaFileServingHandler: () => createElysiaFileServingHandler,
  createExpressAdapter: () => createExpressAdapter,
  createExpressFileServingMiddleware: () => createExpressFileServingMiddleware,
  createFastifyAdapter: () => createFastifyAdapter,
  createFastifyFileServingPlugin: () => createFastifyFileServingPlugin,
  createH3Adapter: () => createH3Adapter,
  createHonoAdapter: () => createHonoAdapter,
  createHonoFileServingMiddleware: () => createHonoFileServingMiddleware,
  createKoaAdapter: () => createKoaAdapter,
  createKoaFileServingMiddleware: () => createKoaFileServingMiddleware,
  ...
  createValidationHooks: () => createValidationHooks,
  decryptQueryString: () => decryptQueryString,
  detectKind: () => detectKind,
  generateRandomString: () => generateRandomString,
  getMimeKind: () => getMimeKind,
  hashString: () => hashString,
  parseBooleanFlag: () => parseBooleanFlag,
  parseIntSafe: () => parseIntSafe,
  parseJsonSafe: () => parseJsonSafe,
  resolveChunkLimit: () => resolveChunkLimit,
  resolveSizeLimit: () => resolveSizeLimit,
  resolveStorageKey: () => resolveStorageKey,
  resolveUploadConfig: () => resolveUploadConfig,
  signData: () => signData,
  toBuffer: () => toBuffer,
...
function resolveUploadConfig(config) {
  ...
  return {
    ...
    janitorIntervalMs: config.janitorIntervalMs ?? 24 * 60 * 60 * 1e3
  };
}
...
var MultipartParser = class {
  ...
  static async parseBuffered(req, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const isChunked = !!(req.headers["content-range"] || req.query?.isChunked);
      ...
      bb.on("file", (fieldname, fileStream, info) => {
        ...
        let fileResolve;
        let fileReject;
        pendingPromises.push(new Promise((r, rej) => {
          fileResolve = r;
          fileReject = rej;
        }));
        let size = 0;
        ...
        let diskStream = null;
        let tempFilePath = void 0;
        if (shouldDumpToDisk) {
          const os5 = require("os");
          const path8 = require("path");
          const fs6 = require("fs");
          tempFilePath = path8.join(os5.tmpdir(), `upload_parse_${Date.now()}_${Math.random().toString(36).substring(2)}.tmp`);
          diskStream = fs6.createWriteStream(tempFilePath);
        }
        ...
        fileStream.on("end", async () => {
          try {
            if (diskStream) {
              await new Promise((res) => diskStream.end(() => res()));
            }
            if (truncated) {
...
var MediaProcessor = class {
  ...
  async *processVideoYieldingFromPath(inputPath, originalMimeType, originalFilename, config) {
    if (!this.canProcessMedia) {
  ...
  async *_encodeVideoYieldingFromPath(inputPath, originalMimeType, originalFilename, config, tmp) {
    ...
    if (config.qualityConfigs && config.qualityConfigs.length > 1) {
      ...
      const thumbnail2 = await thumbnailPromise;
      yield {
        id: primaryQc.id,
        isPrimary: true,
        path: primaryPath,
      ...
      if (remainingQcs.length > 0) {
        const parallelJobs = remainingQcs.map(async (qc) => {
          const outputPath2 = tmp.create(`.${outputFormat}`, qc.id);
          await this.runFFmpegVideo(inputPath, outputPath2, outputFormat, config, qc, threadsPerProcess, remainingQcs.length, sourceBitrateKbps);
          return {
            id: qc.id,
            isPrimary: false,
            path: outputPath2,
    ...
    return new Promise((resolve, reject) => {
      ...
      cmd.on("progress", (progress) => {
        if (config.onProgress) {
          try {
            config.onProgress({ ...progress, variantId: qc?.id ?? "single" });
          } catch {
          }
        }
      }).on("end", () => {
  ...
  async *processAudioYieldingFromPath(inputPath, originalMimeType, originalFilename, config) {
    if (!this.canProcessMedia) {
      console.warn("[MediaProcessor] FFmpeg not available \u2014 returning passthrough.");
      yield {
        id: "single",
...
var MongooseRepository = class _MongooseRepository {
  ...
  static getOrCreateFileModel(connection, extensions, onInit) {
    ...
    const fileSchema = new import_mongoose.Schema(
  ...
  static getOrCreateChunkModel(connection, onInit) {
    ...
    const chunkSchema = new import_mongoose.Schema(
      {
...
var LocalDiskStorageAdapter = class {
  ...
  async writeChunk(fileId, chunkNumber, data, ctx) {
    ...
    await import_fs.promises.writeFile(dest, data);
      ...
      (async () => {
        try {
          for (let i = 0; i < totalChunks; i++) {
            ...
            const buf = await import_fs.promises.readFile(chunkFile);
      ...
      (async () => {
        try {
          for (let i = 0; i < totalChunks; i++) {
            ...
            const ok = writeStream.write(buf);
            if (!ok) await new Promise((r) => writeStream.once("drain", r));
            await import_fs.promises.unlink(chunkFile).catch(() => {
            });
          }
          writeStream.end();
        } catch (err) {
  ...
  async readStream(ref, options) {
    return (0, import_fs.createReadStream)(path3.join(this.rootDir, ref), {
      start: options?.start,
      end: options?.end
    });
  }
  async delete(ref) {
    await import_fs.promises.unlink(path3.join(this.rootDir, ref)).catch(() => {
    });
  }
};
...
var ChunkReadStream = class extends import_stream.Readable {
  current;
  opts;
...
var S3StorageAdapter = class {
  ...
  chunkCachePath(fileId, chunkNumber) {
    return path5.join(this.tempDir, `s3_cache_${fileId}_${chunkNumber}.bin`);
  }
  async cacheChunk(fileId, chunkNumber, data) {
    if (this.cacheStrategy === "memory") {
      ...
      await fs4.promises.mkdir(this.tempDir, { recursive: true });
      await fs4.promises.writeFile(this.chunkCachePath(fileId, chunkNumber), data);
    }
  }
  async readCachedChunk(fileId, chunkNumber) {
    if (this.cacheStrategy === "memory") {
  ...
  async cleanupCache(fileId, totalChunks) {
    if (this.cacheStrategy === "memory") {
      this.memoryCache.delete(fileId);
    } else {
      const count = totalChunks ?? 1e4;
      const toDelete = [];
      for (let i = 0; i < count; i++) {
        const p = this.chunkCachePath(fileId, i);
        toDelete.push(fs4.promises.unlink(p).catch(() => {
        }));
      }
  ...
  async assembleChunksToPath(fileId, totalChunks, ext, ctx) {
    ...
    const writeStream = fs4.createWriteStream(dest);
    await new Promise((resolve, reject) => {
      writeStream.on("error", reject);
      writeStream.on("finish", resolve);
      (async () => {
  ...
  async serveFromDisk(ref, res, start, end, file) {
    ...
    try {
      ...
      if (start !== void 0 && end !== void 0) {
        ...
        const stream = (0, import_fs2.createReadStream)(fullPath, { start, end });
        await res.pipeFrom(stream);
      } else {
        res.status(200);
        res.header("Content-Length", String(stat.size));
        const stream = (0, import_fs2.createReadStream)(fullPath);
        await res.pipeFrom(stream);
      }
    } catch (err) {
      if (err.code === "ENOENT") {
...
var OptimizedDatabaseMediaStream = class extends import_stream2.Readable {
  currentChunk;
  endChunk;
  fileId;
  fileChunkSize;
...
function toNodeReadable(stream) {
  ...
  return import_stream3.Readable.fromWeb(stream);
}
...
var ElysiaNormalizedResponse = class {
  ...
  async pipeFrom(stream) {
    ...
    this.body = import_stream3.Readable.toWeb(stream);
  }
  end() {
  }
  get raw() {
...
var NextjsNormalizedResponse = class {
  ...
  async pipeFrom(stream) {
    this.headers["Content-Type"] = this.headers["Content-Type"] || "application/octet-stream";
    this.body = import_stream4.Readable.toWeb(stream);
  }
  end() {
  }
  toResponse() {
